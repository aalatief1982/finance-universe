...
const OnboardingSlides: React.FC<Props> = ({ onComplete, isSubmitting = false }) => {
  ...
  const hasLoggedFirstSlideRender = React.useRef(false);
  ...
  const markAnimatedOnce = React.useCallback(() => {
    setHasAnimated((previous) => (previous ? previous : true));
  }, []);

  const slideUpClass = hasAnimated ? '' : 'animate-slide-up';
  const scaleInClass = hasAnimated ? '' : 'animate-scale-in';
  const fadeInClass = hasAnimated ? '' : 'animate-fade-in';
            ...
            <div className="flex flex-col flex-1 min-h-0 h-full">
              ...
              >
                <div className="flex flex-col items-center text-center px-4">
                  ...
                  <h1
                    className={`text-2xl sm:text-3xl font-bold text-foreground mb-2 ${slideUpClass}`}
                    onAnimationEnd={i === 0 && !hasAnimated ? markAnimatedOnce : undefined}
                  >
                    {slide.title}
                  </h1>
                  <p className={`text-base sm:text-lg font-medium text-primary mb-2 ${slideUpClass}`} style={!hasAnimated ? { animationDelay: '0.1s' } : undefined}>
                    {slide.subtitle}
                  </p>
                  <p className={`text-sm text-muted-foreground max-w-sm leading-relaxed ${slideUpClass}`} style={!hasAnimated ? { animationDelay: '0.2s' } : undefined}>
                    {slide.description}
                  </p>
                </div>
              </div>
                ...
                <div className="relative w-full max-w-xs flex items-center justify-center" style={{ maxHeight: '42vh' }}>
                  ...
                  <div className="relative bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-3 shadow-xl w-full h-fit flex items-center justify-center">
                    <img
                      ...
                      alt={slide.title}
                      className={`w-full h-auto max-h-[35vh] object-contain rounded-lg ${scaleInClass}`}
                      style={!hasAnimated ? { animationDelay: '0.3s' } : undefined}
                      onLoad={
                        i === 0
                          ? (event) => {
              ...
              <div className="px-4 pt-2 shrink-0" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 2.5rem)' }}>
                {i === slides.length - 1 ? (
                  <div className={`space-y-3 ${slideUpClass}`} style={!hasAnimated ? { animationDelay: '0.4s' } : undefined}>
                    <Button 
                  ...
                  <div className="h-16 flex items-center justify-center">
                    <p className={`text-sm text-muted-foreground ${fadeInClass}`} style={!hasAnimated ? { animationDelay: '0.5s' } : undefined}>
                      Swipe to continue
                    </p>
                  </div>
                )}
