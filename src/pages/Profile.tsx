      ...
      >
        <Card className="flex flex-col items-center space-y-4 p-6 text-center">
          <div className="relative">
            ...
            <button
              ...
              className="absolute bottom-0 ltr:right-0 rtl:left-0 bg-primary text-primary-foreground h-8 w-8 rounded-full flex items-center justify-center cursor-pointer hover:bg-primary/90 transition-colors"
            >
              <Camera size={14} />
            </button>
          </div>
        ...
        <Card className="space-y-3 p-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{t('profile.dangerZoneDesc')}</p>
          </div>

          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" size="sm" className="w-full sm:w-auto">
                <Trash2 className="ltr:mr-1 rtl:ml-1 h-4 w-4" />
        ...
        <DialogContent className="w-[calc(100%-2rem)] max-w-md max-h-[85dvh] overflow-y-auto px-4 pb-[max(env(safe-area-inset-bottom,0px),1rem)] pt-5 sm:px-6">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="fullName">{t('profile.fullName')}<span className="text-red-500 ltr:ml-1 rtl:mr-1">*</span></Label>
              <Input id="fullName" name="fullName" value={editFormData.fullName} onChange={handleInputChange} placeholder={t('profile.enterFullName')} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email">{t('profile.email')}</Label>
              <Input id="email" name="email" type="email" value={editFormData.email} onChange={handleInputChange} placeholder={t('profile.enterEmail')} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="phone">{t('profile.mobile')}</Label>
              <Input id="phone" name="phone" value={editFormData.phone} onChange={handleInputChange} placeholder={t('profile.enterMobile')} />
            </div>
          </div>
