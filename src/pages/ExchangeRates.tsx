...
import { getAllExchangeRates, deleteExchangeRate } from '@/services/ExchangeRateService';
...
const ExchangeRates: React.FC = () => {
  ...
  const refreshRates = () => setRates(getAllExchangeRates());
  ...
  const handleAddNew = () => { setEditingRate(undefined); setDialogOpen(true); };
  const handleEdit = (rate: ExchangeRate) => { setEditingRate(rate); setDialogOpen(true); };
  const handleDeleteClick = (rate: ExchangeRate) => { setRateToDelete(rate); setDeleteConfirmOpen(true); };

  const handleDeleteConfirm = () => {
    if (rateToDelete) {
      const success = deleteExchangeRate(rateToDelete.id);
      if (success) {
        toast({ title: t('toast.exchangeRateDeleted'), description: '' });
        refreshRates();
      } else {
  ...
  return (
    <Layout withPadding={false} fullWidth>
                  ...
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground ltr:ml-2 rtl:mr-2">
                        ({pairRates.length} {pairRates.length !== 1 ? t('fx.ratesCount') : t('fx.rateCount')})
                      </span>
                    </CardTitle>
                  </CardHeader>
      ...
      <ExchangeRateDialog open={dialogOpen} onOpenChange={setDialogOpen} existingRate={editingRate} onSave={() => refreshRates()} />
      ...
      <AlertDialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <AlertDialogContent className="w-[calc(100%-2rem)] max-w-sm">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t('exchangeRates.deleteRateDesc')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
