...
import { computeTemplateConfidence, ConfidenceResult } from './templateConfidence';
...
export function recordTemplateSuccess(
  hash: string,
  sender?: string,
  fromAccount?: string
): void {
  ...
  if (template) {
    ensureTemplateMeta(template);
    template.meta.successCount = (template.meta.successCount || 0) + 1;
    template.meta.usageCount = (template.meta.usageCount || 0) + 1;
    template.meta.lastUsedAt = new Date().toISOString();
    const { score, status } = computeTemplateConfidence(template);
    template.meta.confidenceScore = score;
    template.meta.status = status;
    templates[key] = template;
    saveTemplateBank(templates);
  }
}

export function recordTemplateFallback(
  hash: string,
  sender?: string,
  fromAccount?: string,
  reason?: string
  ...
  if (template) {
    ensureTemplateMeta(template);
    template.meta.fallbackCount = (template.meta.fallbackCount || 0) + 1;
    template.meta.usageCount = (template.meta.usageCount || 0) + 1;
    template.meta.lastUsedAt = new Date().toISOString();
    template.meta.lastFailureAt = new Date().toISOString();
    const { score, status } = computeTemplateConfidence(template);
    template.meta.confidenceScore = score;
    template.meta.status = status;
    if (status === 'deprecated') {
      template.meta.deprecationReason = reason || 'Low success rate';
    }
    templates[key] = template;
    saveTemplateBank(templates);
  }
...
export function getTemplatesForReview(): SmartPasteTemplate[] {
  const templates = loadTemplateBank();
  return Object.values(templates).filter(t => 
    t.meta?.status === 'learning' || 
    (t.meta?.confidenceScore && t.meta.confidenceScore < 80 && t.meta.confidenceScore >= 50)
  );
}

export function approveTemplate(hash: string, sender?: string): void {
  const templates = loadTemplateBank();
  const key = getTemplateKey(sender, undefined, hash);
  const template = templates[key];
  if (template) {
...
export function deprecateTemplate(hash: string, sender?: string, reason?: string): void {
  const templates = loadTemplateBank();
  const key = getTemplateKey(sender, undefined, hash);
  const template = templates[key];
  if (template) {
    template.meta = {
      ...template.meta,
      status: 'deprecated',
      deprecationReason: reason || 'Manually deprecated',
    };
    templates[key] = template;
    saveTemplateBank(templates);
  }
...
export function pruneDeprecatedTemplates(olderThanDays: number = 90): number {
  const templates = loadTemplateBank();
  ...
  for (const key of Object.keys(templates)) {
    ...
    if (t.meta?.status === 'deprecated') {
      const deprecatedAt = t.meta.lastUsedAt ? new Date(t.meta.lastUsedAt).getTime() : 0;
      if (deprecatedAt < cutoff) {
