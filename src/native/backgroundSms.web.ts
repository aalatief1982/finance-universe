...
const BackgroundSmsListenerWebMock: BackgroundSmsListenerPlugin = {
  addListener: async (eventName, listenerFunc) => {
    if (process.env.NODE_ENV === 'development') console.log(`[SMS-Web] Adding listener for event: ${eventName} (simulation)`);
    
    // Return a mock listener handle
    return {
      remove: async () => {
        if (process.env.NODE_ENV === 'development') console.log(`[SMS-Web] Removing listener for event: ${eventName} (simulation)`);
      }
    } as PluginListenerHandle;
  },
  
  checkPermission: async () => {
    if (process.env.NODE_ENV === 'development') console.log('[SMS-Web] Checking permission (simulation)');
  ...
  requestPermission: async () => {
    if (process.env.NODE_ENV === 'development') console.log('[SMS-Web] Requesting permission (simulation)');
  ...
  startListening: async () => {
    if (process.env.NODE_ENV === 'development') console.log('[SMS-Web] Starting to listen for SMS (simulation)');
    // Nothing to do in web environment
    return;
  },
  
  stopListening: async () => {
    if (process.env.NODE_ENV === 'development') console.log('[SMS-Web] Stopping listening for SMS (simulation)');
    // Nothing to do in web environment
    return;
  }
};
