...
interface ExpenseByCategory { name: string; value: number; [key: string]: unknown; }
interface ExpenseBySubcategory { name: string; value: number; [key: string]: unknown; }
interface ExpenseChartProps { expensesByCategory: ExpenseByCategory[]; expensesBySubcategory: ExpenseBySubcategory[]; }

const COLORS = ['#007bff', '#28a745', '#ffc107', '#dc3545', '#6f42c1', '#6c757d'];
const CHART_MARGIN = { top: 20, right: 20, left: 20, bottom: 20 };

type ChartTooltipProps = TooltipProps<number, string>;
interface AxisTickProps { x?: number; y?: number; payload?: { value?: string | number }; }
  ...
  const BarTooltipRenderer = ({ active, payload }: ChartTooltipProps) => {
    ...
    return (
      <div className="bg-popover border border-border p-2 rounded-md shadow-sm text-sm">
        ...
        <p className="text-primary">{formatCurrency(Math.abs(value))}{percent ? ` • ${percent}%` : ''}</p>
      </div>
    );
  };
...
const YAxisTick = ({ x = 0, y = 0, payload }: AxisTickProps) => {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <title>{text}</title>
      <text x={-4} y={0} dy={4} textAnchor="end" className="text-xs fill-foreground">{truncated}</text>
    </g>
  );
};
      ...
      <Card className="border border-border shadow-sm overflow-hidden">
        <CardHeader className="pb-0 flex items-center justify-between">
          <CardTitle className="text-xl font-medium flex items-center gap-1">
            {t('chart.expenseAnalysis')}
            <TooltipProvider>
              <UiTooltip>
                <TooltipTrigger asChild><Info className="w-4 h-4" /></TooltipTrigger>
                <TooltipContent>{t('chart.categorizedView')}</TooltipContent>
              </UiTooltip>
            </TooltipProvider>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="mb-4 border-b">
              <TabsTrigger value="category" className="data-[state=active]:border-b-2 data-[state=active]:border-primary font-medium transition-colors">{t('chart.byCategory')}</TabsTrigger>
              <TabsTrigger value="subcategory" disabled={!hasExpensesBySubcategory} className="data-[state=active]:border-b-2 data-[state=active]:border-primary font-medium transition-colors" title={hasExpensesBySubcategory ? undefined : t('chart.noDataToShow')}>
                {t('chart.subcategories')}
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="category" className="pt-2">
              {hasExpensesByCategory ? (
                limitedCategoryData.length > 1 ? (
                  <div className="h-[300px] w-full" role="img" aria-label={t('chart.byCategory')}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart margin={CHART_MARGIN}>
                        <Pie data={limitedCategoryData} cx="50%" cy="50%" labelLine={false} outerRadius={80} innerRadius={40} fill="#8884d8" dataKey="value" isAnimationActive={true} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                          {limitedCategoryData.map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                        </Pie>
                        <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" className="text-sm fill-foreground">{formatCurrency(totalExpense)}</text>
            ...
            <TabsContent value="subcategory" className="pt-2">
              {hasExpensesBySubcategory ? (
                <div className="h-[300px] w-full" role="img" aria-label={t('chart.subcategories')}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={expensesBySubcategory} margin={CHART_MARGIN}>
                      ...
                      <Bar dataKey="value" radius={[4, 4, 4, 4]} isAnimationActive>
                        {expensesBySubcategory.map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
